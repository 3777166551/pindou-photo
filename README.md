...
- 🧊 **立体组合**：选 2~4 个项目存档堆成多层立体件，逐层调位/层高，3D 预览 + 合并豆单 + 导出预览图
- 📽 **对位投屏**：拼豆辅助里一键进相机，四点校准把当前色"钉"到真实拼豆板上，边看边拼（画面仅本地使用）
- 🧵 **十字绣导出**：图纸一键转 DMC 绣线色号绣图（就近映射 454 色），带针数清单和 14ct 成品尺寸
- 📄 **PDF 图纸导入**：识别图纸入口直接选 PDF 文件，离线渲染后自动识别网格复刻
- 📷 **AR 试摆（假 AR）**：效果图页一键进相机取景，拼好的板子"立"在你面前，转动手机即可环顾（纯预览不可交互，画面仅本地使用；无陀螺仪自动降级固定视角）
