// 移植自 BeadColor.java + BeadPalettes.java
import { brandChart, brandChartCount, brandChartName } from './BrandCharts';

/** 一颗拼豆的颜色定义 */
export class BeadColor {
  /** 色号(1 起,本 APP 内部编号;品牌色板下仅作列表序号) */
  code: number;
  /** 中文名称(品牌色板为官方英文名) */
  name: string;
  /** 颜色值 0xRRGGBB */
  rgb: number;
  /** 品牌官方色号(如 "S-47");通用色板为空串,此时展示内部编号 */
  tag: string;

  constructor(code: number, name: string, rgb: number, tag: string = '') {
    this.code = code;
    this.name = name;
    this.rgb = rgb;
    this.tag = tag;
  }

  /** 展示用色号:品牌色板显示官方色号,通用色板显示内部编号 */
  displayCode(): string {
    return this.tag.length === 0 ? this.code + '号' : this.tag;
  }

  /** 完整展示名:"名称(色号)";品牌色号与名称相同时只显示一次 */
  fullLabel(): string {
    if (this.tag.length > 0 && this.tag === this.name) {
      return this.name;
    }
    return this.name + '(' + this.displayCode() + ')';
  }

  /** 是否是品牌官方色号表里的颜色 */
  hasOfficialCode(): boolean {
    return this.tag.length > 0;
  }
}

/** 通用 4 档色板的名称(品牌色号表名称见 selNames()) */
export const PALETTE_NAMES: string[] = [
    "24色 · 经典",
    "48色 · 标准",
    "90色 · 进阶",
    "120色 · 全彩",
];

/** 通用 4 档之外的选项:各品牌官方色号表 */
export const GENERIC_COUNT: number = 4;

const HEX_T1: number[] = [
    0xFFFFFF, 0xF7F0DD, 0xA8A8A8, 0x4A4A4A, 0x141414, 0xE3242B, 0x9C1C1C, 0xE4007C,
    0xF48FB1, 0xF57C00, 0xF7E01E, 0xF5A623, 0x43A047, 0x1B5E20, 0x26A69A, 0x42A5F5,
    0x1E5AA8, 0x0D2C6B, 0x7B3FA0, 0x795548, 0xC8A17B, 0xF5CBA0, 0x6D4C41, 0x90CAF9,
];
const NAME_T1: string[] = [
    "白色",
    "奶白",
    "灰色",
    "深灰",
    "黑色",
    "大红",
    "深红",
    "玫红",
    "粉红",
    "橙色",
    "柠檬黄",
    "大黄",
    "草绿",
    "深绿",
    "青色",
    "天蓝",
    "宝蓝",
    "深蓝",
    "紫色",
    "棕色",
    "浅棕",
    "肤色",
    "咖啡色",
    "浅蓝",
];

const HEX_T2: number[] = [
    0xD9D9D9, 0x757575, 0x7B1113, 0xA04A3A, 0xFF7F6E, 0xFFB6C1, 0xE91E63, 0xFFB74D,
    0xE65100, 0xFFF59D, 0xFBC02D, 0xB8A233, 0x7CB342, 0x556B2F, 0x00838F, 0x4DD0E1,
    0x039BE5, 0x1A237E, 0x607D8B, 0x512DA8, 0xD1C4E9, 0xA1887F, 0xC3B091, 0xFADCC8,
];
const NAME_T2: string[] = [
    "银灰",
    "中灰",
    "酒红",
    "砖红",
    "珊瑚红",
    "浅粉",
    "艳粉",
    "浅橙",
    "深橙",
    "浅黄",
    "深黄",
    "芥末黄",
    "苹果绿",
    "军绿",
    "深青",
    "浅青",
    "湖蓝",
    "藏蓝",
    "灰蓝",
    "深紫",
    "浅紫",
    "驼色",
    "卡其",
    "浅肤色",
];

const HEX_T3: number[] = [
    0xEAE0C0, 0x2B2B2B, 0xA29A8C, 0x6B7075, 0xEA4A28, 0x5C0E1E, 0xDDBAC2, 0xF2CD9A,
    0xE8A575, 0x9E8B3A, 0xFFBE0B, 0xA9C24A, 0x0E3620, 0xA5DBC8, 0x92A683, 0x63E62E,
    0x13876A, 0xD6E9F8, 0x3949AB, 0x2E7CFF, 0x6A5ACD, 0x452159, 0xC0A5C9, 0x3E2723,
    0x6E2C1B, 0x8B4226, 0xA9713F, 0xD2A24C, 0xC0C7CE, 0xD4AF37, 0xFF6F3C, 0x0277BD,
    0x38623C, 0xE3D5AE, 0xCB6843, 0xA9BFD4, 0xD3ACAF, 0x2F211A, 0x00A651, 0x6E7623,
    0xAD0A63, 0xC4825A,
];
const NAME_T3: string[] = [
    "象牙白",
    "炭灰",
    "暖灰",
    "冷灰",
    "番茄红",
    "深酒红",
    "藕粉",
    "杏色",
    "肤橙",
    "土黄",
    "金黄",
    "黄绿",
    "墨绿",
    "薄荷绿",
    "灰绿",
    "荧光绿",
    "孔雀绿",
    "淡蓝",
    "靛蓝",
    "亮蓝",
    "蓝紫",
    "葡萄紫",
    "藕紫",
    "深棕",
    "栗色",
    "红棕",
    "深肤色",
    "小麦色",
    "银色",
    "金色",
    "荧光橙",
    "深天蓝",
    "森林绿",
    "浅卡其",
    "砖橘",
    "雾蓝",
    "灰粉",
    "黑棕",
    "翠绿",
    "橄榄绿",
    "深玫红",
    "古铜色",
];

const HEX_T4: number[] = [
    0xC96B5D, 0xEF6FA8, 0xFF36A5, 0xD5324F, 0xA04225, 0xFFE45C, 0xE0F53C, 0x00CFFF,
    0x00A98F, 0x01579B, 0x8E7CC3, 0x283593, 0x8D6E63, 0x12203F, 0xB08968, 0x46586A,
    0xDCC2DD, 0x0B5D5A, 0x3F4A21, 0xFFB3A7, 0xFFCC9E, 0x5A6E60, 0xE2C878, 0x9FB2B4,
    0x6F7C3E, 0xA66BB8, 0xE86A50, 0x55C1A5, 0x8E1F4B, 0xFFF9EA,
];
const NAME_T4: string[] = [
    "浅砖红",
    "玫粉",
    "荧光粉",
    "樱桃红",
    "铁锈红",
    "鹅黄",
    "荧光黄",
    "荧光蓝",
    "宝石绿",
    "深湖蓝",
    "灰紫",
    "深靛",
    "咖啡驼",
    "深海军",
    "豆沙色",
    "深灰蓝",
    "香芋紫",
    "深青绿",
    "深橄榄",
    "珊瑚粉",
    "蜜桃色",
    "深灰绿",
    "浅金",
    "青灰",
    "卡其绿",
    "亮紫",
    "深珊瑚",
    "湖水绿",
    "紫红",
    "奶油白",
];

function appendTier(list: BeadColor[], hex: number[], names: string[]): void {
  for (let i = 0; i < hex.length; i++) {
    list.push(new BeadColor(list.length + 1, names[i], hex[i]));
  }
}

/** 通用 120 色 Master 表(4 档依次叠加),构建方式同安卓 buildMaster() */
function buildMaster(): BeadColor[] {
  const list: BeadColor[] = [];
  appendTier(list, HEX_T1, NAME_T1);
  appendTier(list, HEX_T2, NAME_T2);
  appendTier(list, HEX_T3, NAME_T3);
  appendTier(list, HEX_T4, NAME_T4);
  return list;
}

const MASTER: BeadColor[] = buildMaster();

let selNamesCache: string[] | null = null;

/** 色板选择器总数:通用 4 档 + 品牌色号表 */
export function selCount(): number {
  return GENERIC_COUNT + brandChartCount();
}

/**
 * 按选择序号取色板。0~3 = 通用 4 档(24/48/90/120);
 * >=4 = 品牌官方色号表(里面的 BeadColor 带官方 tag)。
 * 与安卓 BeadPalettes.getPalette 语义一致,返回副本。
 */
export function getPalette(sel: number): BeadColor[] {
  if (sel >= GENERIC_COUNT) {
    const j: number = sel - GENERIC_COUNT;
    return brandChart(j);
  }
  const sizes: number[] = [24, 48, 90, 120];
  let n: number = sizes[Math.max(0, Math.min(3, sel))];
  if (n > MASTER.length) {
    n = MASTER.length;
  }
  return MASTER.slice(0, n);
}

/** 色板选择器的全部名称(下拉框直接用),同安卓 selNames() */
export function selNames(): string[] {
  if (selNamesCache !== null) {
    return selNamesCache;
  }
  const n: string[] = [];
  for (let i = 0; i < PALETTE_NAMES.length; i++) {
    n.push(PALETTE_NAMES[i]);
  }
  for (let i = 0; i < brandChartCount(); i++) {
    n.push(brandChartName(i) + '(' + brandChart(i).length + '色)');
  }
  selNamesCache = n;
  return n;
}

export function tierName(sel: number): string {
  const names: string[] = selNames();
  return names[Math.max(0, Math.min(names.length - 1, sel))];
}
