...
"%JAVA_HOME%\bin\javac.exe" -encoding UTF-8 -cp "%AJ%" -sourcepath app\src\main\java -d qa\out ^
  qa\TestColorMath.java qa\TestPatternEngine.java qa\TestPatternPatch.java qa\TestCustomPalette.java qa\TestSymmetry.java qa\TestLineArt.java qa\TestBrandCharts.java qa\TestCrossStitch.java qa\TestBoardProjector.java
if errorlevel 1 (
    echo [COMPILE FAILED]
    exit /b 1
)
...
set FAIL=0
for %%T in (TestColorMath TestPatternEngine TestPatternPatch TestCustomPalette TestSymmetry TestLineArt TestBrandCharts TestBoardProjector TestCrossStitch) do (
    echo ===== running %%T =====
