...
echo === [2/6] aapt2 link ===
"%BT%\aapt2.exe" link -o %OUT%\base.apk -I "%AJ%" --manifest %SRC%\AndroidManifest.xml -R %OUT%\res.zip --java %OUT%\gen -A %SRC%\assets --min-sdk-version 24 --target-sdk-version 34 --version-code 53 --version-name "2.42" --auto-add-overlay
if errorlevel 1 goto :err
...
call "%BT%\apksigner.bat" sign --ks pindou.keystore --ks-key-alias pindou --ks-pass pass:%PINDOU_KS_PASS% --key-pass pass:%PINDOU_KS_PASS% --out %OUT%\PindouPhoto-v2.42.apk %OUT%\aligned.apk
if errorlevel 1 goto :err
call "%BT%\apksigner.bat" verify %OUT%\PindouPhoto-v2.42.apk
if errorlevel 1 goto :err

echo.
echo ============================================
echo  BUILD OK: %CD%\%OUT%\PindouPhoto-v2.42.apk
echo ============================================
exit /b 0
